...
/* ── stub bindings ── */
const bundle = {
  "acme/current.json": JSON.stringify({ version: 3, publishedAt: "2026-07-31T00:00:00Z", files: ["index.html"] }),
  "acme/v3/index.html": "<!doctype html><title>Demo</title><h1>hello</h1>",
  "acme/v3/css/app.css": "body{color:#111}",
};
const env = {
  ...
  TOKENS: { get: async (key) => (key === "token:acme" ? "agx_edge_test_token" : null) },
};

const req = (url, init) => new Request(url, init);

/* ── serving ── */
let res = await worker.fetch(req("https://acme.xvibe.app/"), env);
check("root serves index.html from the current version", res.status === 200 && (await res.text()).includes("hello"));

res = await worker.fetch(req("https://acme.xvibe.app/"), env);
...
res = await worker.fetch(req("https://acme.xvibe.app/css/app.css"), env);
...
res = await worker.fetch(req("https://acme.xvibe.app/dashboard"), env);
check("extensionless route falls back to the app shell (SPA)", res.status === 200 && (await res.text()).includes("hello"));

res = await worker.fetch(req("https://acme.xvibe.app/missing.png"), env);
check("missing asset is a real 404", res.status === 404);

const etag = (await worker.fetch(req("https://acme.xvibe.app/"), env)).headers.get("etag");
res = await worker.fetch(req("https://acme.xvibe.app/", { headers: { "if-none-match": etag } }), env);
...
for (const host of ["https://xvibe.app/", "https://www.xvibe.app/", "https://studio.xvibe.app/x", "https://acme.other.com/"]) {
...
res = await worker.fetch(req("https://acme.xvibe.app/api/v1/guestbook?limit=5", { headers: { "x-user-token": "jwt-abc", cookie: "secret=1" } }), env);
...
res = await worker.fetch(req("https://acme.xvibe.app/index.html", { method: "POST" }), env);
